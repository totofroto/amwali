...
const DEFAULTS={
  appName:'أموالي',
  users:[{id:'u1',name:'توتوفروتو'},{id:'u2',name:'أخي'}],
  adminId:'u1',
  ...
  tx:[], transfers:[], debts:[], budgets:[], recurring:[], goals:[], pin:null,
...
function load(){
  try{
    ...
    if(raw){const d=JSON.parse(raw); return Object.assign(JSON.parse(JSON.stringify(DEFAULTS)),d);}
  }catch(e){}
...
const BGS=[
  ['#0f766e','#134e4a'],['#1e3a8a','#172554'],['#7c2d92','#4a1259'],
  ['#b45309','#7c2d12'],['#be123c','#4c0519'],['#334155','#0f172a'],
  ['#065f46','#022c22'],['#4338ca','#1e1b4b'],['#0369a1','#082f49'],['#57534e','#1c1917'],
];
...
function renderHeader(){
  ...
  document.getElementById('adminChip').textContent=userName(S.adminId);
...
function openTxModal(type,editId){
  ...
  if(editId){
    ...
    document.getElementById('txUser').value=S.adminId;
  }
...
function delTx(id){
  ...
  snapshotLocal(true);
  const item=S.tx.find(t=>t.id===id);
  S.tx=S.tx.filter(t=>t.id!==id); save(); renderTx(); renderHome(); sndDel();
  toastUndo('حُذفت المعاملة 🗑️',()=>{S.tx.push(item); save(); renderTx(); renderHome();});
}
function renderTx(){
  ...
  document.getElementById('txList').innerHTML=list.length?list.map(t=>{
    ...
    return `<div class="item">
      ...
      <div class="info"><div class="t1">${esc(c.name)}</div><div class="t2">${dateAr(t.date)} · ${esc(userName(t.user))}${t.note?' · '+esc(t.note):''}</div></div>
...
function openTransferModal(editId){
  ...
  if(editId){
    ...
    document.getElementById('trFrom').value=S.adminId;
    const other=S.users.find(u=>u.id!==S.adminId);
...
function delTransfer(id){
  ...
  snapshotLocal(true);
  const item=S.transfers.find(t=>t.id===id);
  S.transfers=S.transfers.filter(t=>t.id!==id); save(); renderTransfers(); renderHome(); sndDel();
  toastUndo('حُذفت الحوالة 🗑️',()=>{S.transfers.push(item); save(); renderTransfers(); renderHome();});
}
...
function renderReports(){
  ...
  const COLORS=['#14b8a6','#f59e0b','#ef4444','#3b82f6','#8b5cf6','#ec4899','#22c55e','#eab308','#06b6d4','#f97316','#a855f7','#64748b'];
  if(typeof Chart!=='undefined'){
    ...
    if(labels.length){
      pieChart=new Chart(document.getElementById('pieChart'),{
        ...
        options:{maintainAspectRatio:false,plugins:{legend:{position:'left',labels:{color:getComputedStyle(document.body).getPropertyValue('--text'),font:{family:'Segoe UI'}}}}}
      });
    ...
    barChart=new Chart(document.getElementById('barChart'),{
      ...
      data:{labels:months,datasets:[
        {label:'دخل',data:incArr,backgroundColor:'#22c55e',borderRadius:6},
        {label:'مصروف',data:expArr,backgroundColor:'#ef4444',borderRadius:6}
      ]},
    ...
    netChart=new Chart(document.getElementById('netChart'),{
      ...
      data:{labels:nLabels,datasets:[{label:'صافي الثروة ('+cur+')',data:nData,
        borderColor:'#14b8a6',backgroundColor:'#14b8a633',fill:true,tension:.35,pointRadius:3}]},
      options:{maintainAspectRatio:false,scales:{x:{ticks:{color:tc}},y:{ticks:{color:tc}}},plugins:{legend:{labels:{color:tc}}}}
    });
  }else{
...
function wipeData(){
  if(!confirm('⚠️ سيتم مسح كل البيانات. ستُحفظ نقطة استعادة قبل المسح. متابعة؟'))return;
...
function saveBudget(){
  ...
  S.budgets.push({catId,cur,amount});
...
function delRecurring(id){
  if(!confirm('إيقاف هذه المعاملة المتكررة؟'))return;
  S.recurring=S.recurring.filter(r=>r.id!==id); save(); renderSettings(); toast('تم الإيقاف');
}
...
function saveDebt(){
  ...
  const rec={
    ...
    note:document.getElementById('dbNote').value.trim(), ts:Date.now()
  };
...
function delDebt(id){
  ...
  S.debts=S.debts.filter(d=>d.id!==id); save(); renderDebts(); sndDel();
  toastUndo('حُذف الدين 🗑️',()=>{S.debts.push(item); save(); renderDebts();});
}
...
const COLLS=['tx','transfers','debts','budgets','recurring','goals'];
...
async function cloudPull(silent){
  ...
  try{
    ...
    if(changed){
      saveLocal(); rebuildShadow(d); rerenderAll();
